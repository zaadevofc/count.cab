  ...
  return (
    <>
      <section className={`${className} ${tab == 'default' ? 'opacity-100' : 'opacity-0 hidden'} duration-1000 flex-col gap-5 flex h-fit md:sticky top-6 z-10 max-w-xs w-full`}>
        <div className={`flex flex-col p-5 gap-8 bg-white rounded-2xl shadow-md shadow-rose-500/20 border border-black`}>
          <div className='flex flex-row items-center justify-between'>
            ...
            <Tooltip showArrow={true} placement='left' size='sm' color='danger' content="Logout?">
              <div className='md:cursor-pointer' onClick={() => setMDelOpen(true)}>
                <LuLogOut className='stroke-red-500' />
              </div>
            </Tooltip>
          </div>
          <div className='flex flex-col items-center gap-3'>
            {status != 'loading' ? (
              <Badge content={<LuCheck className='stroke-white' />} shape="circle" isOneChar color="success" size='md' className='top-1 right-1 text-[10px]'>
                <div>
                  <img className='w-28 rounded-2xl drop-shadow-lg' src={session?.token?.picture} alt="Kounter APi" />
                </div>
              </Badge>
            ) : (
            ...
            <div className='flex flex-col items-center'>
              ...
              {status != 'loading' ? (
                <h1 className='text-sm border border-slate-300 bg-primary-50 mt-1 text-primary-400 px-2 rounded-lg'>{session?.token?.email}</h1>
              ) : (
              ...
              {(status != 'loading' && allKounter) ? (
                <h1 className='text-sm border border-slate-300 bg-rose-50 text-rose-500 mt-1.5 px-2 rounded-lg'>Kounter Limit : {isKounterMaks - Number(allKounter?.length)}</h1>
              ) : (
                <Skeleton className="rounded-2xl mt-1 border-2 border-gray-300">
                  <div className="w-24 h-5 rounded-2xl"></div>
                </Skeleton>
        ...
        <div className='flex flex-col gap-2'>
          <Skeleton isLoaded={status != 'loading'} className="rounded-2xl border-2 border-gray-300">
            <Button onClick={() => router.push('/dash?tab=add')} color="primary" startContent={<LuPlus className='stroke-white' />} className='w-full'>
              Buat Kounter
            </Button>
          </Skeleton>
          <Skeleton isLoaded={status != 'loading'} className="rounded-2xl border-2 border-gray-300">
        ...
        <div className={`shadow-md shadow-orange-500/20 bg-white border border-black rounded-xl`}>
          <Accordion>
            <AccordionItem key="1" title={
              ...
              <div className='flex flex-row items-center pb-2 gap-3'>
                <Link href='https://saweria.co/zaadevofc' target='_blank' className='flex flex-row gap-2 items-center bg-yellow-500/50 border border-yellow-600 w-fit p-2 rounded-xl'>
                  <div className='w-8'>
                    <img className='w-full drop-shadow-md' src="https://saweria.co/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fcapy_happy.603c7293.svg&w=384&q=75" alt="Kounter APi" />
                  </div>
                ...
                <Link href='https://trakteer.id/zaadevofc/tip' target='_blank' className='flex flex-row gap-2 items-center bg-red-500/50 border border-red-600 w-fit p-2 rounded-xl'>
                  <div className='w-6'>
                    <img className='w-full drop-shadow-md' src="https://trakteer.id/images/v2/trakteer-logo-circle.png" alt="Kounter APi" />
                  </div>
